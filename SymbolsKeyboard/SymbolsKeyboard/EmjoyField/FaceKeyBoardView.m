...
@property (nonatomic, strong)FacesManager * FManager;
...
//    NSMutableArray * arrFaces = (NSMutableArray *)[defaults objectForKey:@"RecentlyFaces"];
...
//    }
//    block传值
//    NSArray *ar = [NSArray arrayWithArray:self.arrFace];
//    id dic = ar[button.tag];
////    NSDictionary *dic = ar[button.tag];
//    NSLog(@"%@",dic);
    self.block(self.arrFace[button.tag][@"cht"],button.tag);
}
...
//- (void)setToolBar
//{
//    self.toolBar = [[UIToolbar alloc] initWithFrame:CGRectMake(0, self.scFace.frame.size.height, ScreenWidth, ToolBarHeight)];
